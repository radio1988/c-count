...
from ccount.img_utils import read_czi, parse_image_obj
from ccount.blob_utils import visualize_blobs_on_img, visualize_blob_compare
from ccount.blob_utils import get_blob_statistics
...
if len(sys.argv) != 6:
    ...
    npy_file = sys.argv[
        3]  # locs or crops file (locs corresponding to circles in jpg-labeled), e.g. '../npy/1unitEpo_1-Stitching-01.1.crops.clas.npy.gz'
...
# read locs
npy_based_locs = load_blobs(npy_file)  # either crops or locs are converted to locs
...
for i in range(len(npy_based_locs)):
    ...
    count = count_orange_pixels_in_circle(img, x, y, r)
...
print('jpg based label statistics:')
stats_dot = get_blob_statistics(dot_based_locs)
# print('circle based label statistics:')
# stats_cir = get_blob_statistics(circle_based_locs)

if npy_based_locs.shape[1] > 3:
    print("input blob file's label statistics:")
    stats_npy = get_blob_statistics(npy_based_locs)
